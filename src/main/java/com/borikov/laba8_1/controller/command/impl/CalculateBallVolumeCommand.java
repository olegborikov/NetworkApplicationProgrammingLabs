...
public class CalculateBallVolumeCommand implements Command {
    ...
    public String execute(HttpServletRequest request) {
        ...
        try {
            double ballVolume = figureService.calculateBallVolume(radius, accuracy);
            request.setAttribute(RequestParameter.RESULT, ballVolume);
            page = PagePath.RESULT;
        } catch (IncorrectDataException e) {
            LOGGER.log(Level.ERROR, "Error while calculate ball volume", e);
            request.setAttribute(RequestParameter.INCORRECT_DATA, true);
            page = PagePath.BALL_VOLUME;
        }
        return page;
    }
}
