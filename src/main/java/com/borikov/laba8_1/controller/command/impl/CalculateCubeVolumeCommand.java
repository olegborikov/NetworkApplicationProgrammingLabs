...
public class CalculateCubeVolumeCommand implements Command {
    ...
    public String execute(HttpServletRequest request) {
        ...
        try {
            double ballVolume = figureService.calculateCubeVolume(side, accuracy);
            request.setAttribute(RequestParameter.RESULT, ballVolume);
            page = PagePath.RESULT;
        } catch (IncorrectDataException e) {
            LOGGER.log(Level.ERROR, "Error while calculate cube volume", e);
            request.setAttribute(RequestParameter.INCORRECT_DATA, true);
            page = PagePath.CUBE_VOLUME;
        }
        return page;
    }
}
